...
class CurrySpider(scrapy.Spider):
    ...
    """
    name = "curry"
    allowed_domains = ["curry.virginia.edu"]
    start_urls = (
        'http://curry.virginia.edu/about/directory',
    )
    ...
    def parse(self, response):
        ...
        links = sel.xpath('//div[@class="directory-results"]/ul/li/a/@href').extract()
        for link in links:
            p_link = 'http://www.curry.virginia.edu%s' %link
    ...
    def parse_profile_page(self, response):
        ...
        name = sel.xpath('//h1[@class="page-title serif-heading"]/text()').extract()
        if name:
