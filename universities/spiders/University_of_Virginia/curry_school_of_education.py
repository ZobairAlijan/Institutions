...
class CommerceSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://curry.virginia.edu/about/directory#results',
    )
    ...
    def parse(self, response):
        ...
        marketing_sel = the_sel.xpath('//div[@class="directory-results"]')

        for comm_sel in marketing_sel:
            the_commerce = University()

            name = comm_sel.xpath('//ul/li[@class="facultyTeaser"]/a/strong/text()')
            if name:
