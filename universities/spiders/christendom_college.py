...
class CnuSpider(scrapy.Spider):
    name = "cnu"
    allowed_domains = ["cnu.edu"]
    start_urls = (
        'http://cnu.edu/experts/index.asp',
    )
    ...
    def parse(self, response):
        ...
        links = sel.xpath('//div[@class="text-content"]/div/ul/li/a/@href').extract()
        for link in links:
            p_link = 'http://www.cnu.edu%s' %link
    ...
    def parse_profile_page(self, response):
        ...
        name = sel.xpath('//div[@id="printCopy"]//text()').extract()
        if name:
            item['name'] = name
